...
function getStructure (d, allowText) {
  ...
  try {    
    ...
    return getUndefinedStructure(d, allowText);
  }
  ...
  var res = {tagName: d.tagName, addedStyles: d.addedStyles, display: maybeStyle, float: maybeFloat, children: [], nodeType: d.nodeType};
  for(var i = 0; i < otherAttrs.length; i++)
  {
    try {
    if(d.getAttribute(otherAttrs[i]))
        res[otherAttrs[i].toLowerCase()] = d.getAttribute(otherAttrs[i]);
    } catch (e) {
  ...
  if (d.childNodes) {
    for (var i = 0; i < d.childNodes.length; i++) {
      if (!allowText && d.childNodes[i].nodeType == 3) continue;
      try {
        var c = getStructure(d.childNodes[i], allowText);
...
function getStructureMain (d, frames, allowText) {
  var res = getStructure(d, allowText);
  if (frames) {
    for (var i = 0; i < frames.length; i++) 
      try {
      res.children.push(getStructure(frames[i].document.getElementsByTagName('body')[0], allowText));
...
function getUndefinedStructure (d, allowText) {
  ...
  return (
    ...
    (d.childNodes.length == 1) ? getStructure(d.childNodes[0], allowText)  : (function () {
           
      var res = {tagName: 'unknown', addedStyles: d.addedStyles, display: 'block', float: false, children: [], nodeType: d.nodeType};
      for(var i = 0; i < otherAttrs.length; i++)
      {
        if(d.getAttribute(otherAttrs[i]))
            res[otherAttrs[i].toLowerCase()] = d.getAttribute(otherAttrs[i]);
...
function convertPage(root, allowText){
  var body = Sizzle(root)[0];
  return normalize(getStructureMain(body, null, allowText));
}
