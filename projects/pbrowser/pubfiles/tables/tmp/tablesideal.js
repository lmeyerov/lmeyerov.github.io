...
function visit_cols_1(node) {
  ...
  logger.log("    last init cols_colnum_last: " + getAttribSafe(node, "cols_colnum_last"));
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      setAttribSafe(child, "borderc", getInputAttribSafe(node, "borderc", '#FF0000'));
    }
  })();
...
function visit_col_6(node) {
  ...
  setAttribSafe(node, "render", col_render(getInputAttribSafe(node, "borderc"), getAttribSafe(node, "canvas"), getAttribSafe(node, "computedwidth"), getAttribSafe(node, "absy"), getAttribSafe(node, "absx"), getAttribSafe(node, "tablecontentheight")));
