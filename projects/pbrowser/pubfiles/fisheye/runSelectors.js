function runSelectors(styles, nodes){
    styleRules =  getStyles(styles);
    for (var rule in styleRules)
    {
        var matches = [];  
        if(styleRules[rule].selectorText)
            matches = Sizzle(styleRules[rule].selectorText);
        else
            matches.push(styleRules[rule].appliesTo);
     
        for (var nodeI in matches){
            if (!(matches[nodeI] instanceof HTMLElement)) continue;
            if (nodes.indexOf(matches[nodeI]) == -1) continue;
            node = matches[nodeI];
    ...
    //var nodes = Sizzle("*");
    for(var i in nodes){
        if (!(nodes[i] instanceof HTMLElement)) continue;
            ...
            for(var j = 0; j < nodes[i].addedCSSRules.length; j++)
            {
                ...
                {                
                    ...
                    if(typeof(val) == "string" && isNaN(Number(k)) && val  != "" && k != "cssText"){
                        nodes[i].addedStyles[k.toLowerCase()] = val;
                    }
                }
            }
        }
...
function splitPass(selectors, splitString){
    ...
    {
        if (typeof selectors[i] != "string") continue;
...
function parseStyle(styleText){
    ...
    for(var i in styles){
        if (typeof styles[i] != "string") continue;
